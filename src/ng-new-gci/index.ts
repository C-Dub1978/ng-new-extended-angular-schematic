...
import { logging, normalize, strings } from '@angular-devkit/core';
import {
  apply,
  chain,
  externalSchematic,
...
import { AddToModuleContext } from './util/add-to-module-context';
...
function _updatePackageJson(
  ...
  return (tree: Tree): Tree => {
    ...
    if (dependencies.includes('cypress')) {
      json.scripts = {
        ...json.scripts,
        'cy:open-local': `cypress open /${options.projectName}/cypress/integration/local/**/*.ts`,
        'cy:open-remote': `cypress open /${options.projectName}/cypress/integration/remote/**/*.ts`,
        'cy:run': 'cypress run',
      };
    }
...
function _setTreeStructure(
  ...
  return (tree: Tree) => {
    ...
    const angularOverridePath = !!_options?.pwa
    ...
    const updatedIndex = tree.read(indexPath)?.toString('utf-8');

    if (!tree.exists(path) || !updatedIndex) {
      throw new SchematicsException('Couldnt locate index.html');
    }
    ...
    if (!!_options.dependencies.includes('cypress')) {
      ...
      files.forEach((path: string) => {
        if (path === cypressJsonPath) {
          tree.create(
            normalize(path),
            JSON.stringify(cypressJsonFile, null, 2)
          );
        } else if (path === fixtureFilePath || path === supportFilePath) {
          tree.create(normalize(path), '{}');
        } else {
          tree.create(normalize(path), '/// <reference types="Cypress" />\n{}');
        }
      });
    }
    tree = _addImport(
